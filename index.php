 ...
 $uniq_name = date("YmdHis").session_id() . "." . $extension; //ユニークファイル名作成
  ...
  if ( is_uploaded_file( $tmp_path ) ) {
    if ( move_uploaded_file( $tmp_path, "upload/".$uniq_name ) ) {
      ...
      //３．データ登録SQL作成
      $stmt = $pdo->prepare("INSERT INTO map_info (id, lat, lon, img, comment,input_date )VALUES(NULL, :lat, :lon, :img, :comment, sysdate())");
  ...
  <nav class="navbar navbar-default navbar-static-top">
    <div class="container">
      <ul class="nav navbar-nav">
        ...
        <li><a href="map_view.php">カメラ／写真選択</a></li>
      </ul>
    </div>
  </nav>
</header>
